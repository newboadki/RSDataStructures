...
/// A reference type wrapper to provide indirect storage for direct node access in the heap.
/// This allows efficient updates and lookups by value, which is useful for operations like priority updates.
fileprivate class NodeDirectAccecssIndirectStorage<T: KeyValuePair> {
    /// Dictionary mapping value (`T.V`) to the corresponding heap node (`BasicBinaryHeap<T>`).
    var directAccessToNodes: Dictionary<T.V, BasicBinaryHeap<T>> = [:]
    
    /// Subscript to access or modify the node associated with a given value.
    ///
    /// - Parameter position: The value (`T.V`) whose node is to be accessed or set.
    /// - Returns: The heap node associated with the given value, or `nil` if not found.
    ...
    /// Removes the node associated with the specified value from the storage.
    ///
    /// - Parameter value: The value (`T.V`) whose node should be removed.
